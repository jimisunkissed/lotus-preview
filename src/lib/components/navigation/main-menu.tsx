...
export function MainMenu(): React.ReactNode {
  ...
  return (
    <>
      <div className="fixed z-50 flex left-6 sm:left-10 top-[35px] sm:top-[50px]">
        <Button variant="ghost" className="group flex gap-3.5 p-0 hover:bg-transparent" onClick={() => setOpenMenu(!openMenu)}>
          ...
          <div
            className={cn(
              'hidden sm:flex text-[15px] items-center gap-2',
              animated ? 'transition-all duration-300' : '',
      ...
      <div
        className={cn(
          'fixed top-0 left-0 z-40 flex flex-col h-[100dvh] w-full sm:w-[500px] pt-32 px-8 sm:px-10 gap-4 bg-black',
          animated ? 'transition-all duration-300' : '',
      ...
      >
        {menuList.map((menu, i) => (
          <Link
            key={i}
            href={menu.path}
            className="w-full text-5xl sm:text-6xl lg:text-7xl font-medium text-white hover:text-neutral-300 transition-all"
